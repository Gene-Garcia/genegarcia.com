...
function CardButton({ id, name, color, onClick }) {
  ...
  return (
    <div
      className={`flex flex-row sm:flex-col items-center sm:items-start transition duration-400 ease-linear shadow hover:shadow-2xl border-l-4 ${theme.root} rounded p-3 md:p-4 gap-3 sm:gap-1 md:gap-2 sm:h-24 md:h-32 xl:h-40`}
      onClick={() => onClick(id)}
    >
      <div
        className={`h-8 w-8 md:h-10 md:w-10 rounded-full transition duration-300 ${theme.circle}`}
      ></div>
